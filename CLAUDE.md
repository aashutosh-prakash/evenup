...
**Money math.** Expense `amount` is stored as a **plain number** (e.g. `120.5`); **integer cents live only inside `src/lib/settle.js`**, which does all balance math there to avoid float drift. `settle()` is a greedy who-pays-whom minimizer that always reconciles to zero. Don't reimplement balance math elsewhere — compute via `computeBalances`/`settle`/`computePaidTotals`.
...
- Import lz-string as a **default** (`import LZString from 'lz-string'`), not named exports — it's CommonJS, so named imports resolve under Vite but **break in plain Node** (e.g. one-off `node` scripts).
...
- **UI labels differ from code names**: the **Members** panel is `PeoplePanel` (state key `people`), and **Settlements** is the settle-up output — don't expect a "Members"/"Settlements" file.
